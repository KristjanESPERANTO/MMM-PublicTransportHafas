...
| `fetchRetries`                    | <p>How many retries should be used for transient fetch errors per direction.</p><p>**Type:** `integer`<br>**Example:** `3`<br>**Default value:** `2`<br>**Allowed range:** `0` to `5`</p><p>**Note:** Retries are only used for transient network/stream failures (for example timeouts or premature close errors). Backoff is applied between attempts. Set to `0` to disable retries.</p>|
| `discardSocketErrorThreshold`     | <p>How many consecutive socket errors should be tolerated before showing an error message.</p><p>**Type:** `integer`<br>**Example:** `5` (Tolerate 5 consecutive errors before showing error message.)<br>**Default value:** `3`</p><p>**Note:** When temporary network issues occur, the module will keep displaying the last successfully fetched departures instead of showing an error immediately. The number of socket issues is displayed in the "Last update" line (e.g., "Update (socket issues: 2)"). Only after exceeding this threshold will an error message be displayed. Set to `0` to show errors immediately. Errors below the threshold are logged as warnings, errors above as errors.</p>|
| `direction`                       | <p>**Deprecated, use `directions` instead**<br>An ID of a station. It is used to display only those departures heading to this station.</p><p>**Type:** `string`<br>**Example:** `"954609"`<br>**Default value:** `""`</p><p>**Note:** It is not necessary to find the ID of the end station. Just use the next station which is on the route you are after.</p><p>**Note!** This option doesn't work anymore using the default `db` profile. Please try a regional profile instead. Check out the issue [#193](https://github.com/KristjanESPERANTO/MMM-PublicTransportHafas/issues/193#issuecomment-2599743884) for more details.</p>|
| `directions`                       | <p>IDs of a stations. It is used to display only those departures heading to these stations.</p><p>**Type:** `array`<br>**Example:** `["954609"]`<br>**Default value:** `[]`</p><p>**Note:** It is not necessary to find the ID of the end station. Just use the next station which is on the route you are after.</p><p>**Note!** This option doesn't work anymore using the default `db` profile. Please try a regional profile instead. Check out the issue [#193](https://github.com/KristjanESPERANTO/MMM-PublicTransportHafas/issues/193#issuecomment-2599743884) for more details.</p>|
...
| `excludeDirections`              | <p>An array of destination names you want to exclude from the displayed departures.</p><p>**Type:** `array`<br>**Example:** `[ "Central Station", "Plaza" ]`<br>**Default value:** `[]`</p><p>**Note:** The comparison is case-sensitive and matches the `direction` value returned by HAFAS exactly.</p>|
| `platformsToShow`                | <p>An array of platform names to filter departures. Only departures from these platforms will be displayed.</p><p>**Type:** `array`<br>**Example:** `[ "1", "2", "A" ]`<br>**Default value:** `[]` (shows all platforms)</p><p>**Note:** The filter uses the actual platform if available, otherwise falls back to the planned platform. The comparison is case-sensitive.</p>|
...
| `toggleAbsoluteTimeInterval`      | <p>Automatically switch between absolute and relative departure time display at a fixed interval.</p><p>**Type:** `integer`<br>**Default value:** `0` (disabled)<br>**Unit:** seconds</p><p>**Note:** When set to a value greater than `0`, the display alternates between absolute time (e.g. `10:15+0`) and relative time (e.g. `in 5 minutes`) every n seconds. The starting mode is determined by `showAbsoluteTime`. Set to `0` to disable toggling.</p> |
...
| `showWarningRemarks`              | <p>A boolean value indicating whether warnings attached to a trip should be shown.</p><p>**Type:** `boolean`<br>**Default value:** `true`<br>**Possible values:** `true` and `false`</p><p>**Note:** If set to `true` and a warning is attached to a trip it will be shown in an additional row. The warning text scrolls as a marquee. The scroll speed can be adjusted with `warningRemarksScrollSpeed`.</p> |
...
| `warningRemarksScrollSpeed`       | <p>The scroll speed of warning remarks in characters per second.</p><p>**Type:** `integer`<br>**Default value:** `10`</p><p>**Note:** Higher values result in faster scrolling. A value of `10` is a comfortable reading pace for most people. Set to a higher value like `25` for a faster scroll, or a lower value like `5` for a slower one.</p>|
...
```javascript
    {
      ...
      config: {
        ...
        fetchRetries: 2,                   // Retry transient network/stream errors per direction.
